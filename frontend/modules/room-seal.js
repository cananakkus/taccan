...
export function renderRoomSeal(roomCode) {
  ...
  let svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="48" height="48">`;
  svg += `<path d="${shieldPath}" fill="none" stroke="#6b3a2a" stroke-width="2" ${borderAttr} opacity="0.45"/>`;
  svg += `<path d="${shieldPath}" fill="#6b3a2a" opacity="0.05"/>`;
  svg += `<path d="${iconPath}" fill="#6b3a2a" opacity="0.3"/>`;
  svg += `</svg>`;
