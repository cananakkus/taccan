import { ui } from './ui.js';

const SHIELDS = [
  // Shield shape 1: classic pointed
  'M50 5 L90 25 L90 60 Q90 85 50 95 Q10 85 10 60 L10 25 Z',
  // Shield shape 2: rounded
  'M50 5 Q90 5 90 35 L90 60 Q90 90 50 95 Q10 90 10 60 L10 35 Q10 5 50 5 Z',
  // Shield shape 3: flat top
  'M15 10 L85 10 L90 30 L90 65 Q90 90 50 95 Q10 90 10 65 L10 30 Z',
];

const ICONS = [
  // Eye
  'M50 40 Q35 40 28 50 Q35 60 50 60 Q65 60 72 50 Q65 40 50 40 Z M50 44 A6 6 0 1 1 50 56 A6 6 0 1 1 50 44 Z',
  // Key
  'M40 35 A10 10 0 1 1 40 55 A10 10 0 1 1 40 35 Z M48 50 L70 50 L70 58 L65 58 L65 54 L60 54 L60 58 L55 58 L55 50',
  // Star
  'M50 20 L56 38 L75 38 L60 49 L66 68 L50 56 L34 68 L40 49 L25 38 L44 38 Z',
  // Crown
  'M20 65 L20 40 L35 52 L50 30 L65 52 L80 40 L80 65 Z',
  // Compass
  'M50 15 L55 45 L85 50 L55 55 L50 85 L45 55 L15 50 L45 45 Z',
];

const BORDERS = [
  'none',
  'stroke-dasharray="4 2"',
  'stroke-dasharray="8 3"',
  'stroke-dasharray="2 2 6 2"',
];

export function renderRoomSeal(roomCode) {
  if (!ui.roomSealContainer) return;

  const hash = hashCode(roomCode);
  const shieldIdx = Math.abs(hash) % SHIELDS.length;
  const iconIdx = Math.abs(hash >> 4) % ICONS.length;
  const borderIdx = Math.abs(hash >> 8) % BORDERS.length;

  const shieldPath = SHIELDS[shieldIdx];
  const iconPath = ICONS[iconIdx];
  const borderAttr = BORDERS[borderIdx];

  let svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="56" height="56">`;
  svg += `<path d="${shieldPath}" fill="none" stroke="#7a4b1f" stroke-width="2" ${borderAttr}/>`;
  svg += `<path d="${shieldPath}" fill="#7a4b1f" opacity="0.08"/>`;
  svg += `<path d="${iconPath}" fill="#7a4b1f" opacity="0.7"/>`;
  svg += `</svg>`;

  ui.roomSealContainer.innerHTML = svg;
}

function hashCode(str) {
  let hash = 0;
  for (let i = 0; i < str.length; i++) {
    hash = ((hash << 5) - hash + str.charCodeAt(i)) | 0;
  }
  return hash;
}
